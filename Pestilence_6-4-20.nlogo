...
without-interruption[
  ...
  ask people [set vaccinated false  ifelse random 100 < Poor [set poorPer true][set poorPer false] ifelse random 100 < Employment_Rate [set employedPer true][set employedPer false] set tested false]
  set continue false
  ;loop[
  ...
  ;]
           ask patches[
           if (abs(pxcor) = 50 and abs(pycor) <= 50 and abs(pycor) >= 28) or                           ;; Hospitals/work/school
         (abs(pycor) = 50 and abs(pxcor) <= 50 and abs(pxcor) > 28)
         [set pcolor black]
        if (abs(pxcor) = 28 and abs(pycor) <= 50 and abs(pycor) >= 28) or                           ;; Hospitals
         (abs(pycor) = 28 and abs(pxcor) <= 50 and abs(pxcor) > 28)
         [set pcolor black]
       if (pxcor > 28 and pxcor < 50 and abs(pycor) > 28 and abs(pycor) < 50)
              [set pcolor yellow + 4]
       if (abs(pycor) > 35 and abs(pycor) < 45 and pxcor > 37 and pxcor < 41) or         ;; Red Cross
            (abs(pycor) > 38 and abs(pycor) < 42 and pxcor > 34 and pxcor < 44)
               [set pcolor red]
       if (pxcor < -28 and pxcor > -50 and (pycor) > 28 and (pycor) < 50)
               [set pcolor brown + 3]  ;[if (pcolor > 6 and pcolor < 10)                   ;school
       if (pxcor < -28 and pxcor > -50 and (pycor) < -28 and (pycor) > -50)
              [ifelse pcolor > 59 [set pcolor white - .1]
                 [set pcolor black + .001] ;[if pcolor > 54 and pcolor < 58
              set pcolor violet + 3];if pcolor > 9.9 or (pcolor > 6 and pcolor < 9.87)   ;work
    ]
]
end
...
if mouse-down?
    [ if [pcolor] of patch mouse-xcor mouse-ycor = white [set X mouse-xcor set Y mouse-ycor
            while [not ( is-number? ([travelers] of patch X Y))]                    ;;Make sure a number is entered to avoid errors
        [carefully[ask patch X Y [set travelers read-from-string user-input "Travelers"]][ask patch X Y [set travelers "-"] user-message "Invalid Input"]]
            while [not ( is-number? ([meanderers] of patch X Y))]
        [carefully[ask patch X Y [set meanderers read-from-string user-input "Meanderers"]][ask patch X Y [set meanderers "-"] user-message "Invalid Input"]]
            while [not ( is-number? ([%Infected] of patch X Y))]
        [carefully[ask patch X Y [set %Infected read-from-string user-input "Percent Infected (0-100)"]][ask patch X Y [set %Infected "-"] user-message "Invalid Input"]]
            while [not ( is-number? ([density] of patch X Y))]
        [carefully[ask patch X Y [set density read-from-string user-input "Density (0-100)"]][ask patch X Y [set density "-"] user-message "Invalid Input"]]
    ] ]
;setup-pops
end
...
if continue[stop]   ;; Forces quit when 'Go' is clicked
    if mouse-down?
  [ if [pcolor] of patch mouse-xcor mouse-ycor = white [set X mouse-xcor set Y mouse-ycor
    while [not ( is-number? ([source_delay] of patch X Y))]
              [carefully[ask patch X Y [set source_delay read-from-string user-input "Source Delay (year)"]][ask patch X Y [set source_delay "-"] user-message "Invalid Input"]]
    while [not ( is-number? ([source_intensity] of patch X Y))]
              [carefully[ask patch X Y [set source_intensity read-from-string user-input "Source Intensity (0-100)"]][ask patch X Y [set source_intensity "-"] user-message "Invalid Input"]]
    while [not ( is-number? ([source_size] of patch X Y))]
              [carefully[ask patch X Y [set source_size read-from-string user-input "Source Size (0-100)"]][ask patch X Y [set source_size "-"] user-message "Invalid Input"]]
  ...
  ;loop[
    ifelse count people > 0[ ;continue and
      ask people [
        change-dir
        move
      if time mod 24 = 9 and employedPer and (time < Shelter_In_Place or time > Resume_Activity)
        [set homeX xcor set homeY ycor go-to-school-work]
      if time mod 24 = 17 and employedPer
        [go-to-home]
      if pcolor = 49  or pcolor = 15 ;on hospital grounds
        [setxy homeX homeY]
      if firstCase > -999 and random 100 < Disease_Testing and pcolor != 38 and pcolor != 118 and tested != true;; if currently at school/work don't go to hospital  if not willing to retest
        [set homeX xcor set homeY ycor ifelse random 100 > 50 [setxy 40 -40 set tested true if shade-of? turquoise color [set color (color - 70) set testCarrierToInfected testCarrierToInfected + 1] ] [setxy 40 40 set tested true] if shade-of? turquoise color [set color (color - 70) set testCarrierToInfected testCarrierToInfected + 1]] ;randomly go to one of the 2 hospitals
      infect
      if time = Shelter_In_Place
        [ifelse shape = "default" [set iwasatraveler true] [set iwasatraveler false]set shape "circle"]
      if time = Resume_Activity
        [if iwasatraveler [set shape "default"]]
      if time mod 6570 = 0 ;9 months in hours
        [reproduce]
      if time mod 8760 = 0 ;1 year in hours
        [age]
...
to advance-time
  ...
  if time = Source_Delay [sprout Source_Intensity[
      ...
      let mySource_Size Source_Size
       set shape "arrow" repeat mySource_Size [infect-area check-approp-source fd 1] die]]
  ]
end
...
to go-to-school-work
  ...
  ifelse (color > 106) or ;Healthy Adult
     (shade-of? red color and color > 16) or ;Infected Adult
     (shade-of? turquoise color and color > 76) ;Carrier Adult
  [setxy -40 -40]
  [setxy -40 40]
end
...
to disp-header
  file-type "Background: "                      file-type Background                     file-type ",,,"
  file-type ",People_Map: "                     file-type People_Map                     file-print ","
  ...
  file-print ""
  file-print "Time,Total,Healthy,Infected,Carriers,Susceptible,Infect Dead,Natural Dead,Births,Resistant,Vaccinated,Pharm Cured,% Area Infected,Case:Fatality,Cases"
...
to update-file
  ...
  ifelse dead > 0[file-type cases / dead][file-type 0] file-type ","
  file-type cases file-print ","
