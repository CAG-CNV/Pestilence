...
without-interruption[
  ...
  file-close
  if File_Output
   [file-close carefully [setup-file][user-message "The Excel SS must be closed while exporting data." stop]]
  setup-maps
 ...
 carefully[
  if File_Output
    [file-close
     ...
     disp-header
     update-file]]
  [user-message "The Excel SS must be closed while exporting data." stop]
  ;loop[
...
to advance-time
  ...
  if time = Source_Delay [sprout Source_Intensity[
       set shape "arrow" repeat Source_Size [infect-area check-approp-source fd 1] die]]
  ]
end
